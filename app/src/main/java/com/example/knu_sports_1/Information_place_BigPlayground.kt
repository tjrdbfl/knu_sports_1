...
class Information_place_BigPlayground : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val binding =ActivityInformationPlaceBigPlaygroundBinding.inflate(layoutInflater)
        ...
        tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            ...
            override fun onTabSelected(tab: TabLayout.Tab?) {
                ...
                when (tab?.text) {
                    "이용 안내" -> transaction.replace(R.id.tabContent, Information_place_BigPlayground_fragmentOne())
                    "예약 안내" -> transaction.replace(R.id.tabContent, Information_place_BigPlayground_fragmentTwo())
                    "취소/환급 규정"->transaction.replace(R.id.tabContent, Information_place_BigPlayground_fragmentThree())
                }
                transaction.commit()
            }
